...
Club_path = os.getcwd()  # Où il y a le .csv
Club_Tag = '2CURLUUGQ'
...
if not os.path.exists(os.path.join(Club_path, 'Club Member Infos.csv')):
    ...
    for i in Old_Club_List:
        if date in i[0]:
            sys.exit('Already made Today')  # Can be removed if you want to update the csv every time
            pass
...
for member in tqdm(Infos_club['members'], desc='Getting info '):
    Club_List.append([date + ' - '+hour, i+1, member['name'],
                     member['role'], date, member['trophies'], 0, 0, member['tag']])
    ...
    Additional_Info = [Infos_player["3vs3Victories"], Infos_player["soloVictories"],
                       Infos_player["duoVictories"], Infos_player["expPoints"], Infos_player["highestTrophies"]]
...
csv_list = ['Date', 'Rang', 'Nom', 'Role', 'Ancienneté', 'Trophées', 'Mégapig Fait',
            'Tickets Utilisés', 'Player Tag', '3v3 Win', 'Solo Win', 'Duo Win', 'Tot Exp', 'Max Trophées']
...
if not rank_fr == 'Unranked':
    ...
    response_ranking_global = requests.get(
        api_url_ranking_global, headers=headers)

    if not response_ranking_global.status_code == 200:
        sys.exit(
            f'Ranking Global : Error{response_ranking_global.status_code}')
...
with open(os.path.join(Club_path, 'Club Infos.csv'), 'a', newline='', encoding="utf-8") as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(["Date", "Nom du Club", "Club Tag", "Description", "Trophées Total", "Nombre de membres",
                        "Membres Sortant", "Membres Entrant", "Difference de TR", "Rang FR", "Rang Global", "Top Member", "Bot Member"])
    csv_writer.writerow([date+' - '+hour, Infos_club["name"], Infos_club["tag"], Infos_club["description"], Infos_club["trophies"], len(Infos_club["members"]),
                        quitting_members, new_members, tot_tr_diff, rank_fr, rank_global, Infos_club['members'][0]["name"], Infos_club['members'][-1]["name"]])
